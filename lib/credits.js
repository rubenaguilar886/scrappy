...
async function getUserAccess(userId, rubro, ciudad) {
  ...
  // 2. Paquete Ciudad activo para este rubro+ciudad exacto
  const pack = await db.query(
    `SELECT id, credits_total, credits_used, expires_at
     FROM city_packs
     WHERE user_id = $1 AND lower(rubro) = $2 AND lower(ciudad) = $3
       AND expires_at > now() AND credits_used < credits_total
     ORDER BY purchased_at DESC LIMIT 1`,
    [userId, normalize(rubro), normalize(ciudad)]
  );
  if (pack.rows.length) {
    const p = pack.rows[0];
    return {
...
module.exports = { getUserAccess, applyAccessToResults, businessKey, TRIAL_CREDITS };
