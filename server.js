...
app.post("/api/scrape", async (req, res) => {
  ...
  try {
    ...
    if (currentJob.metrics) {
      currentJob.metrics.pctWhatsApp = Math.round(results.filter(b => b.whatsapp).length / n * 100);
      currentJob.metrics.pctEmail    = Math.round(results.filter(b => b.email).length    / n * 100);
      currentJob.metrics.pctWeb      = Math.round(results.filter(b => b.website).length  / n * 100);
    }
    currentJob.results = results;
    currentJob.progress = { stage: "done", message: "Listo. " + results.length + " negocios unicos encontrados.", total: results.length, current: results.length };
  } catch (error) {
    ...
    currentJob.error = friendly;
    currentJob.progress = { stage: "error", message: friendly };
  } finally {
...
app.post("/api/scrape-produce", async (req, res) => {
  ...
  try {
    ...
    const businesses = await readProduceCSV({
      ...
      onProgress:  (p) => { currentJob.progress = p; },
    });

    if (!businesses.length) {
      currentJob.results = [];
      currentJob.progress = { stage: "done", message: "No se encontraron empresas con esos filtros." };
      currentJob.running = false;
      return;
    }

    currentJob.progress = { stage: "csv_done", message: businesses.length + " empresas cargadas del CSV.", total: businesses.length, current: businesses.length };
    ...
    currentJob.results  = results;
    currentJob.metrics  = { total: results.length, found, noContact, sunat: sunatMeta };
    currentJob.progress = {
      stage: "done",
      message: enrich
    ...
    currentJob.error    = err.message || "Error inesperado";
    currentJob.progress = { stage: "error", message: currentJob.error };
  } finally {
    currentJob.running = false;
  }
});
...
app.get("/api/export/csv-produce", (_req, res) => {
  if (!currentJob?.results?.length || currentJob.source !== "produce") {
    return res.status(404).json({ error: "No hay resultados PRODUCE para exportar." });
  }
  const rows = currentJob.results.map(toProduceSheetRow);
  const headers = Object.keys(rows[0]);
  const escape = (v) => {
    const s = String(v ?? "");
...
app.get("/api/export/csv", (_req, res) => {
  if (!currentJob?.results?.length) return res.status(404).json({ error: "No hay resultados para exportar." });
  const csv = toCsv(currentJob.results);
  res.setHeader("Content-Type", "text/csv; charset=utf-8");
...
app.get("/api/export/json", (_req, res) => {
  ...
  res.send(JSON.stringify(currentJob.results, null, 2));
});
